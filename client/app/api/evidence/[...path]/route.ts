...
export async function GET(
  ...
  try {
    // 1. AUTH CHECK - Verify user has session
    const cookieStore = await cookies()
    const sessionCookie = cookieStore.get('sb-session') || cookieStore.get('supabase-auth-token')
    
    // For client-side (Cek Status), we allow access if they have the report access key in query
    // For admin, we check session
    const { searchParams } = new URL(request.url)
    const accessKey = searchParams.get('key')
    
    // Simple auth: either has session cookie OR provides valid access key
    const hasAuth = !!sessionCookie || !!accessKey
